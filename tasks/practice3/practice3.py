from csv import reader as csvrd
        ...
        if word not in d:
            for symbol in word:
                ...
                if symbol not in letters:
                    DIGIT_IN_WORD = True
                    break
            if not DIGIT_IN_WORD and word_lenght >= 2:
                d[word] = 1
        else:
            d[word] += 1
    return d
...
def csv_reader(header: str) -> int:
    ...
    k = 0
    answer = []
    with open(get_path_to_file()) as tasks:
        csv_readed = csvrd(tasks)
        for index, raw in enumerate(csv_readed):
            if index == 0:
                k = raw.index(header)
            else:
                raw[k] = raw[k].replace('\n', '')
                if raw[k] not in answer:
                    answer.append(raw[k])
    return len(answer)
