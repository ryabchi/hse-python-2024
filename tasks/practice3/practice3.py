...
def count_words(text: str) -> Dict[str, int]:
    ...
    my_dict = {}
    for word in text:
        if len(set(word) & set("0123456789")) == 0:
...
def get_cashback(operations: List[Dict[str, Any]], special_category: List[str]) -> float:
    ...
    """
    result = 0
    for my_dict in operations:
        if my_dict["category"] in special_category:
            result += float(my_dict["amount"]) * 0.05
        else:
            result += float(my_dict["amount"]) * 0.01
    return result
...
def csv_reader(header: str) -> int:
    ...
    # пиши свой код здесь
    csv_file = open(get_path_to_file())
    ...
    for row in reader:
        for i in range(len(headers)):
            my_dict[headers[i]].append(row[i])

    return len(set(my_dict[header]))
