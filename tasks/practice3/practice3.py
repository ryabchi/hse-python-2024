...
def exp_list(numbers: List[int], exp: int) -> List[int]:
    ...
    res_numbers = [x ** exp for x in numbers]
...
def get_cashback(operations: List[Dict[str, Any]], special_category: List[str]) -> float:
    ...
    return reduce(lambda cashback, operation:
                  cashback + operation['amount'] * (0.05 if operation['category'] in special_category else 0.01),
                  operations, 0)
