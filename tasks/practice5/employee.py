...
from tasks.practice5.exception import NoSuchPositionError, EmployeeError
...
class Employee:
    ...
    def __eq__(self, other: object) -> bool:
        ...
        if not isinstance(other, Employee):
            raise TypeError("Сравнение может быть только между работниками")
        try:
            return get_position_level(self.position) == get_position_level(other.position)
        except NoSuchPositionError as e:
            raise ValueError(f"Ошибка сравнения позиции: {e}")
    def __str__(self):
        """
