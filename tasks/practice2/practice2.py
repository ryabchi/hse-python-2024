...
def get_amount() -> float:
    ...
    # пиши код здесь
    amount = round(random.random() * 999900 + 100, 2)
    return amount
...
def is_phone_correct(phone_number: str) -> bool:
    ...
    # пиши код здесь
    if phone_number[0] == '+' and len(phone_number) == 12 and phone_number[1:].isdigit() and phone_number[1] == '7':
        result = True
    else:
        result = False
    return result
...
def moderate_text(text: str, uncultured_words: Iterable[str]) -> str:
    ...
    for (index, _) in enumerate(words):
        if words[index] in uncultured_words:
            words[index] = '#' * len(words[index])
        elif words[index][-1] == '!' and words[index][:-1] in uncultured_words:
            words[index] = '#' * (len(words[index]) - 1) + '!'
        else:
