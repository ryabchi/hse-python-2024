...
def get_amount() -> float:
    ...
    # пиши код здесь
    amount = round(rnd(100, 1000000), 2)
    return amount
...
def is_phone_correct(phone_number: str) -> bool:
    ...
    """
    allow_symbols = [str(symbol) for symbol in range(10)]
...
def create_request_for_loan(user_info: str) -> str:
    ...
    # пиши код здесь
    user_info_partition = user_info.split(',')
    result = f'Фамилия: {user_info_partition[0]}\nИмя: {user_info_partition[1]}\nОтчество: {user_info_partition[2]}\n' \
             f'Дата рождения: {user_info_partition[3]}\nЗапрошенная сумма: {user_info_partition[4]}'
    return result
