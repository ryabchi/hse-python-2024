...
def get_amount() -> float:
    ...
    amount = round(random.random() * 1000000 + 100, 2)
    return amount
...
def moderate_text(text: str, uncultured_words: Iterable[str]) -> str:
    ...
    get_words = list(map(lambda x: x.strip(), text.lower().split()))
    for i in range(len(get_words)):
        ...
        if word in uncultured_words:
            word = '#' * len(word)
        elif i == len(get_words) - 1 and word[-1] == '!' and word[:-1] in uncultured_words:
            word = '#' * (len(word) - 1) + '!'
        get_words[i] = word
    result = (' '.join(get_words)).capitalize()
    return result
...
def create_request_for_loan(user_info: str) -> str:
    ...
    surname, name, otchestvo, birthday, summ = user_info.split(',')
    result = f'Фамилия: {surname}\nИмя: {name}\nОтчество: {otchestvo}\nДата рождения: {birthday}\nЗапрошенная сумма: {summ}'
    return result
