...
def is_phone_correct(phone_number: str) -> bool:
    ...
    pattern = '+7' + '[0-9]' * 10
    result = fnmatch.fnmatch(phone_number, pattern)
...
def moderate_text(text: str, uncultured_words: Iterable[str]) -> str:
    ...
    result = ''.join(list(filter(lambda x: x not in ['"', "'"], result)))
...
def create_request_for_loan(user_info: str) -> str:
    ...
    sur, name, pat, data, count = user_info.split(',')
    result = f'Фамилия: {sur}\nИмя: {name}\nОтчество: {pat}\nДата рождения: {data}\nЗапрошенная сумма: {count}'
