...
def greet_user(name: str) -> str:
    ...
    greeting = f'Hello, my friend {name}!'
    return greeting
...
def is_phone_correct(phone_number: str) -> bool:
    ...
    result = phone_number[0:2] == "+7" and phone_number[2:].isdigit() and len(phone_number) == 12
    return result
...
def moderate_text(text: str, uncultured_words: Iterable[str]) -> str:
    ...
    result = ''.join(filter(lambda sym: sym not in ['"', "'"], text))
...
def create_request_for_loan(user_info: str) -> str:
    ...
    dictionary = {'Фамилия': list_user_info[0], 'Имя': list_user_info[1], 'Отчество': list_user_info[2],
                  'Дата рождения': list_user_info[3], 'Запрошенная сумма': list_user_info[4]}
    for i in dictionary:
