...
def is_phone_correct(phone_number: str) -> bool:
    ...
    result = (phone_number[0:2] == '+7') \
             and (all([symbol in valid_symbols for symbol in phone_number[2:]])) \
...
def create_request_for_loan(user_info: str) -> str:
    ...
    # пиши код здесь
    user_info_attributes = [attribute for attribute in user_info.split(',')]
    result = f'Фамилия: {user_info_attributes[0]}\n' \
             f'Имя: {user_info_attributes[1]}\n' \
             f'Отчество: {user_info_attributes[2]}\n' \
             f'Дата рождения: {user_info_attributes[3]}\n' \
             f'Запрошенная сумма: {user_info_attributes[4]}'
    return result
