...
def is_phone_correct(phone_number: str) -> bool:
    ...
    # пиши код здесь
    result = True if re.match(r"\+7\d{10}$", phone_number) else False
    return result
...
def moderate_text(text: str, uncultured_words: Iterable[str]) -> str:
    ...
    # пиши код здесь
    text = text.lower()
    replace_words_map = {word: len(word) * "#" for word in uncultured_words}
    replace_words_map["'"] = ""
    replace_words_map['"'] = ""
    group_reg = "|".join(map(re.escape, replace_words_map))
    result = re.sub(group_reg, lambda w: replace_words_map[w.group(0)], text)
...
def create_request_for_loan(user_info: str) -> str:
    ...
    result = f"""Фамилия: {split_info[0]}
Имя: {split_info[1]}
Отчество: {split_info[2]}
Дата рождения: {split_info[3]}
Запрошенная сумма: {split_info[4]}"""
    return result
