...
def greet_user(name: str) -> str:
    ...
    # пиши код здесь
    greeting = f"Hi there, {name}! Welcome to the chat! :)"
    return greeting
...
def is_phone_correct(phone_number: str) -> bool:
    ...
    # пиши код здесь
    if phone_number[:2] == "+7" and phone_number[2:].isdigit() and len(phone_number) == 12:
        result = True
    else:
        result = False
...
def create_request_for_loan(user_info: str) -> str:
    ...
    result = f"Фамилия: {words[0]}\nИмя: {words[1]}\nОтчество: {words[2]}\nДата рождения: {words[3]}\nЗапрошенная сумма: {words[4]}"
    return result
