...
def get_amount() -> float:
    ...
    amount = round(random.uniform(100.00, 1_000_000.00), 2)
...
def is_phone_correct(phone_number: str) -> bool:
    ...
    result = (len(phone_number) == 12 and phone_number[:2] == '+7' and phone_number[1:].isnumeric())
...
def moderate_text(text: str, uncultured_words: Iterable[str]) -> str:
    ...
    # пиши код здесь
    result = text.strip()
    result = (result[:1].upper() + result[1:].lower()).replace('"', '').replace("'", '')
...
def create_request_for_loan(user_info: str) -> str:
    ...
    result = f'''Фамилия: {user_info_arr[0]}
Имя: {user_info_arr[1]}
Отчество: {user_info_arr[2]}
Дата рождения: {user_info_arr[3]}
Запрошенная сумма: {user_info_arr[4]}'''
