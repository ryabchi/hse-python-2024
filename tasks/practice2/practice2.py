...
def is_phone_correct(phone_number: str) -> bool:
    ...
    # пиши код здесь
    result = len(phone_number) == 12 and phone_number[0] == '+' and phone_number[1] == '7' and all([x.isdigit() for x in phone_number[2:]])
    return result
...
def moderate_text(text: str, uncultured_words: Iterable[str]) -> str:
    ...
    # пиши код здесь
    text = text.replace('"', "").replace("'", "").strip().capitalize()
    for stop_word in uncultured_words:
...
def create_request_for_loan(user_info: str) -> str:
    ...
    # пиши код здесь
    components = user_info.split(',')
    result = f"Фамилия: {components[0]}\n" + f"Имя: {components[1]}\n" + f"Отчество: {components[2]}\n" + f"Дата рождения: {components[3]}\n" + f"Запрошенная сумма: {components[4]}"
    return result
